# Spec status — derived, do not hand-edit

> Generated by `tools/spec_status.py`. Regenerate after any spec change;
> `--check` fails if this file has drifted. Rationale in `docs/DECISION_LOG.md` **RD-003**.

**7 specs** — 3 flagged for review.

## ⚠ Needs attention

These are disagreements between what a spec says and what the tree contains.

### `specs/lobby-flow/` — 🟢 active

- **MISSING** — open tasks name 1 file(s) not in the tree: ui-kit.test.ts

### `specs/shell/` — 🟡 dormant

- **MISSING** — open tasks name 1 file(s) not in the tree: render.test.ts

### `specs/visual-direction/` — 🟡 dormant

- **MISSING** — open tasks name 13 file(s) not in the tree: src/client/src/kit/textures.ts, textures.test.ts, kit-rules.test.ts, face.test.ts
- **LIKELY-SHIPPED** — 1 open task(s) name only files that already exist — check whether the work shipped and the box was never ticked: T4

## All specs

| spec | status | done | open | superseded | last touched | flags |
|---|---|---:|---:|---:|---|---|
| `lobby-flow` | 🟢 active | 10 | 2 | 0 | 2026-08-31 | MISSING |
| `shell` | 🟡 dormant | 18 | 1 | 0 | 2026-08-31 | MISSING |
| `visual-direction` | 🟡 dormant | 0 | 16 | 3 | 2026-08-31 | MISSING, LIKELY-SHIPPED |
| `minigame-falling-floor` | ✅ done | 7 | 0 | 0 | 2026-08-30 | — |
| `minigame-hot-potato` | ✅ done | 11 | 0 | 0 | 2026-08-30 | — |
| `minigame-scramble` | ✅ done | 11 | 0 | 0 | 2026-08-30 | — |
| `minigame-sweepers` | ✅ done | 10 | 0 | 0 | 2026-08-30 | — |
