# Spec status — derived, do not hand-edit

> Generated by `tools/spec_status.py`. Regenerate after any spec change;
> `--check` fails if this file has drifted. Rationale in `docs/DECISION_LOG.md` **RD-003**.

**14 specs** — 4 flagged for review.

## ⚠ Needs attention

These are disagreements between what a spec says and what the tree contains.

### `specs/action-button/` — 🟡 dormant

- **MISSING** — open tasks name 1 file(s) not in the tree: src/client/src/ui/icons.ts
- **LIKELY-SHIPPED** — 1 open task(s) name only files that already exist — check whether the work shipped and the box was never ticked: T2

### `specs/audio/` — 🟡 dormant

- **MISSING** — open tasks name 2 file(s) not in the tree: sound.test.ts, src/client/src/kit/sound.ts

### `specs/player-collision/` — 🟡 dormant

- **MISSING** — open tasks name 2 file(s) not in the tree: collide.test.ts, src/shared/src/sim/collide.ts
- **LIKELY-SHIPPED** — 1 open task(s) name only files that already exist — check whether the work shipped and the box was never ticked: T3

### `specs/shell/` — 🟡 dormant

- **MISSING** — open tasks name 1 file(s) not in the tree: render.test.ts

## All specs

| spec | status | done | open | superseded | last touched | flags |
|---|---|---:|---:|---:|---|---|
| `action-button` | 🟡 dormant | 0 | 7 | 0 | 2026-08-31 | MISSING, LIKELY-SHIPPED |
| `audio` | 🟡 dormant | 0 | 5 | 0 | 2026-08-31 | MISSING |
| `player-collision` | 🟡 dormant | 0 | 5 | 0 | 2026-08-31 | MISSING, LIKELY-SHIPPED |
| `shell` | 🟡 dormant | 18 | 1 | 0 | 2026-08-31 | MISSING |
| `arena-framing` | 🟢 active | 5 | 1 | 0 | 2026-08-31 | — |
| `lobby-flow` | 🟡 dormant | 15 | 1 | 0 | 2026-08-31 | — |
| `responsiveness` | 🟡 dormant | 3 | 1 | 1 | 2026-08-31 | — |
| `touch-controls` | 🟡 dormant | 8 | 1 | 0 | 2026-08-31 | — |
| `visual-direction` | 🟡 dormant | 15 | 1 | 3 | 2026-08-31 | — |
| `minigame-falling-floor` | ✅ done | 7 | 0 | 0 | 2026-08-31 | — |
| `minigame-hot-potato` | ✅ done | 11 | 0 | 0 | 2026-08-31 | — |
| `minigame-scramble` | ✅ done | 11 | 0 | 0 | 2026-08-31 | — |
| `minigame-sweepers` | ✅ done | 10 | 0 | 0 | 2026-08-31 | — |
| `round-brief` | ✅ done | 3 | 0 | 0 | 2026-08-31 | — |
